...
namespace Eigen
{
  ...
  namespace internal
  {
    ...
    public:
      ...
      private:
        ...
        {
          while ((bool(*this)) &&
                 internal::isMuchSmallerThan(value(),
                                             m_view.reference(),
                                             m_view.epsilon()))
            {
              EvalIterator::operator++();
   ...
   * assembled dense Matrix \c D to a SparseMatrix \c S: \code MatrixXd D(n,m);
   ...
   * (A*B).pruned(ref); C = (A*B).pruned(ref,epsilon); \endcode where \c ref is
   * a meaningful non zero reference value.
   * */
  template <typename Derived>
  const SparseView<Derived>
