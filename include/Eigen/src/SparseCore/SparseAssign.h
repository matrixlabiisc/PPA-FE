...
namespace Eigen
{
  ...
  namespace internal
  {
    ...
    // Specialization for dense ?= dense +/- sparse and dense ?= sparse +/-
    // dense
    ...
    struct assignment_from_dense_op_sparse
    {
      ...
      run(DstXprType &                    dst,
          const CwiseBinaryOp<internal::scalar_sum_op<Scalar, Scalar>,
                              const Lhs,
                              const Rhs> &src,
      ...
      // Specialization for dense1 = sparse - dense2; -> dense1 = -dense2;
      // dense1 += sparse;
      template <typename Lhs, typename Rhs, typename Scalar>
      static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE typename internal::enable_if<
        internal::is_same<typename internal::evaluator_traits<Rhs>::Shape,
...
#define EIGEN_CATCH_ASSIGN_DENSE_OP_SPARSE(ASSIGN_OP, BINOP, ASSIGN_OP2)      \
  ...
  {}
    ...
    EIGEN_CATCH_ASSIGN_DENSE_OP_SPARSE(add_assign_op,
                                       scalar_sum_op,
                                       add_assign_op);
    EIGEN_CATCH_ASSIGN_DENSE_OP_SPARSE(sub_assign_op,
                                       scalar_sum_op,
                                       sub_assign_op);

    EIGEN_CATCH_ASSIGN_DENSE_OP_SPARSE(assign_op,
    ...
    struct Assignment<DstXprType, SrcXprType, Functor, Diagonal2Sparse>
    {
      ...
      static void
      run(SparseMatrix<Scalar, Options, StorageIndex> &dst,
          const SrcXprType &                           src,
      ...
      static void
      run(SparseMatrixBase<DstDerived> &dst,
          const SrcXprType &            src,
          const internal::assign_op<typename DstXprType::Scalar,
                                    typename SrcXprType::Scalar> & /*func*/)
      {
