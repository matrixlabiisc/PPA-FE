...
#define EIGEN_CHOLMOD_SPECIALIZE1(ret, name, t1, a1)                       \
  ...
  {                                                                        \
    ...
    template <>
    inline cholmod_dense *
    cm_solve<SuiteSparse_long>(int             sys,
                               cholmod_factor &L,
                               cholmod_dense & B,
                               cholmod_common &Common)
    {
    ...
    template <>
    inline cholmod_sparse *
    cm_spsolve<SuiteSparse_long>(int             sys,
                                 cholmod_factor &L,
                                 cholmod_sparse &B,
                                 cholmod_common &Common)
    {
...
#undef EIGEN_CHOLMOD_SPECIALIZE1
  ...
  public:
    ...
    _solve_impl(const MatrixBase<Rhs> &b, MatrixBase<Dest> &dest) const
    {
      ...
      cholmod_dense *x_cd = internal::cm_solve<StorageIndex>(CHOLMOD_A,
                                                             *m_cholmodFactor,
                                                             b_cd,
                                                             m_cholmod);
      if (!x_cd)
   ...
   * very high order 2D FEM. The sparse matrix A must be selfadjoint and
