...
namespace Eigen
{
  namespace internal
  {
     ...
     * and manipulate the supernodes during the factorization and solution phase
     * of Sparse LU. Only the lower triangular matrix has supernodes.
     *
      ...
      for (Index k = 0; k <= nsuper(); k++)
        {
          ...
          if (nsupc == 1)
          ...
          else
            {
              ...
              new (&A) Map<const Matrix<Scalar, Dynamic, Dynamic, ColMajor>,
                           0,
                           OuterStride<>>(&(Lval[luptr + nsupc]),
                                          nrow,
                                          nsupc,
                                          OuterStride<>(lda));
