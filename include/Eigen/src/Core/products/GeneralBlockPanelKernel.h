...
#undef EIGEN_SET_DEFAULT_L3_CACHE_SIZE
    ...
    /** \internal */
    inline void
    manage_caching_sizes(Action          action,
                         std::ptrdiff_t *l1,
                         std::ptrdiff_t *l2,
                         std::ptrdiff_t *l3)
    {
      ...
      if (action == SetAction)
        {
          // set the cpu cache size and cache all block sizes from a global
          // cache size in byte
     ...
     *
     * Given a m x k times k x n matrix product of scalar types \c LhsScalar and
     * \c RhsScalar, this function computes the blocking size parameters along
     * the respective dimensions for matrix products and related algorithms. The
    ...
    template <typename LhsScalar,
              ...
              typename Index>
    void
    evaluateProductBlockingSizesHeuristic(Index &k,
                                          Index &m,
                                          Index &n,
                                          Index  num_threads = 1)
    {
      ...
      // Explanations:
      // Let's recall that the product algorithms form mc x kc vertical panels
      // A' on the lhs and kc x nc blocks B' on the rhs. B' has to fit into
      // L2/L3 cache. Moreover, A' is processed per mr x kc horizontal small
      // panels where mr is the blocking size along the m dimension at the
      // register level. This small horizontal panel has to stay within L1
      // cache.
      ...
      if (num_threads > 1)
        {
          ...
          if (l3 > l2)
            {
              // l3 is shared between all cores, so we'll give each thread its
              // own chunk of l3.
              const Index m_cache =
      ...
      else
        {
          ...
          // horizontal panel of size mr x kc of the lhs plus a vertical panel
          // of kc x nr of the rhs both fits within L1 cache. We also include a
          // register-level block of the result (mx x nr). (In an ideal world
          // only the lhs panel would stay in L1) Moreover, kc has to be a
          // multiple of 8 to be compatible with loop peeling, leading to a
          // maximum blocking size of:
          const Index max_kc =
...
// of L3 shared among 4 cores.
          ...
          // Here, nc is chosen such that a block of kc x nc of the rhs fit
          // within half of L2. The second half is implicitly reserved to access
          // the result and lhs coefficients. When k<max_kc, then nc can
          // arbitrarily growth. In practice, it seems to be fruitful to limit
          // this growth: we bound nc to growth by a factor x1.5. However, if
          // the entire lhs block fit within L1, then we are not going to block
          // on the rows at all, and it becomes fruitful to keep the packed rhs
          ...
          Index nc =
            numext::mini<Index>(actual_l2 / (2 * k * sizeof(RhsScalar)),
                                max_nc) &
            (~(Traits::nr - 1));
          if (n > nc)
            {
              ...
              //    Here we allow one more sweep if this gives us a perfect
              //    match, thus the commented "-1"
              n = (n % nc) == 0 ?
                    nc :
          ...
          else if (old_k == k)
            {
              ...
              if (problem_size <= 1024)
              ...
              else if (l3 != 0 && problem_size <= 32768)
                {
                  ...
                  // kept in L2 Let's choose m such that lhs's block fit in 1/3
                  // of L2
                  actual_lm = l2;
     ...
     * blocking size along the same dimension. \param[in,out] m Input: the
     * number of rows of the left hand side. Output: the blocking size along the
     * same dimension. \param[in,out] n Input: the number of columns of the
     * right hand side. Output: the blocking size along the same dimension.
     *
     * Given a m x k times k x n matrix product of scalar types \c LhsScalar and
     * \c RhsScalar, this function computes the blocking size parameters along
     * the respective dimensions for matrix products and related algorithms.
     *
    ...
    template <typename LhsScalar,
              ...
              typename Index>
    void
    computeProductBlockingSizes(Index &k,
                                Index &m,
                                Index &n,
                                Index  num_threads = 1)
    {
      if (!useSpecificBlockingSizes(k, m, n))
        {
          evaluateProductBlockingSizesHeuristic<LhsScalar,
                                                RhsScalar,
                                                KcFactor,
    ...
    computeProductBlockingSizes(Index &k,
                                ...
                                Index  num_threads = 1)
    {
      computeProductBlockingSizes<LhsScalar, RhsScalar, 1, Index>(k,
                                                                  m,
                                                                  n,
                                                                  num_threads);
    }
    ...
    public:
      typedef typename conditional<remaining_registers >= 4,
                                   RhsPacketx4,
                                   RhsPacket>::type type;
    };
...
#define PACKET_DECL_COND(name, packet_size)     \
  typedef typename packet_conditional<          \
    ...
    typename unpacket_traits<                   \
      typename packet_traits<name##Scalar>::half>::half>::type name##Packet
...
#define PACKET_DECL_COND_SCALAR(packet_size)                                \
  typedef typename packet_conditional<                                      \
     ...
     * end combine them: swap the second and addsub them cf*cf : same but with
     * 2x4 blocks cplx*real : unpack rhs to constant packets, ... real*cplx :
     * load lhs as (a0,a0,a1,a1), and mul as usual
     */
    template <typename _LhsScalar,
              typename _RhsScalar,
              bool _ConjLhs,
  ...
  !defined(EIGEN_VECTORIZE_ALTIVEC) && !defined(EIGEN_VECTORIZE_VSX)
      ...
      loadRhsQuad(const RhsScalar *b, RhsPacket &dest) const
      {
        loadRhsQuad_impl(b,
                         dest,
                         typename conditional<RhsPacketSize == 16,
                                              true_type,
                                              false_type>::type());
      }

      EIGEN_STRONG_INLINE void
      loadRhsQuad_impl(const RhsScalar *b,
                       RhsPacket &      dest,
      ...
      template <typename LhsPacketType,
                ...
                typename AccPacketType>
      EIGEN_STRONG_INLINE void
      madd_impl(const LhsPacketType &a,
                const RhsPacketType &b,
                AccPacketType &      c,
                RhsPacketType &      tmp,
                const true_type &) const
      ...
      typename enable_if<unpacket_traits<RealPacket>::size == 16>::type * = 0)
    {
      ...
      RealScalar                               r[4] = {numext::real(b[0]),
                         numext::real(b[0]),
                         numext::real(b[1]),
                         numext::real(b[1])};
      RealScalar                               i[4] = {numext::imag(b[0]),
                         numext::imag(b[0]),
                         numext::imag(b[1]),
                         numext::imag(b[1])};
    ...
    public:
      ...
      EIGEN_STRONG_INLINE void
      acc(const DoublePacket<RealPacketType> &c,
          const ResPacketType &               alpha,
          ResPacketType &                     r) const
      {
        // assemble c
...
#undef PACKET_DECL_COND
    ...
    template <typename LhsScalar,
              ...
              bool ConjugateLhs,
              bool ConjugateRhs>
    struct last_row_process_16_packets<LhsScalar,
                                       RhsScalar,
                                       Index,
                                       DataMapper,
                                       mr,
      ...
      if (mr >= 3 * Traits::LhsProgress)
        {
          ...
          // panel of the lhs (3*Traits::LhsProgress x depth) and on each
          // largest micro vertical panel of the rhs (depth * nr). Blocking
          // sizes, i.e., 'depth' has been computed so that the micro horizontal
          // panel of the lhs fit in L1. However, if depth is too small, we can
          // extend the number of rows of these horizontal panels. This actual
          // number of rows is computed as follow:
          ...
          // params larger than what our known l1 cache size suggests we should
          // be using: either because our known l1 cache size is inaccurate
          // (e.g. on Android, we can only guess), or because we are testing
          // specific blocking sizes.
                  ...
                  for (Index i = i1; i < actual_panel_end; i += 3 * LhsProgress)
                    {
                      ...
                      // gets res block as register
                      AccPacket C0, C1, C2, C3, C4, C5, C6, C7, C8, C9, C10,
                        C11;
                      traits.initAcc(C0);
                      traits.initAcc(C1);
                      traits.initAcc(C2);
                  ...
                  if ((SwappedTraits::LhsProgress % 4) == 0 &&
                       ...
                       SResPacketQuarterSize == nr))
                    {
                      ...
                      if (SwappedTraits::LhsProgress == 8)
                        ...
                        {
                          // Special case where we have to first reduce the
                          // accumulation register C0. We specialize the block
                          // in template form, so that LhsProgress < 16 paths
                          ...
                          else if (HasHalf && psize == HalfPacketSize)
                            {
                              ...
                              for (int p = 0; p < psize; ++p)
                                kernel_half.packet[p] =
                                  lhs.template loadPacket<HalfPacket>(i + p + m,
                                                                      k);
                          ...
                          else if (HasQuarter && psize == QuarterPacketSize)
                            {
                              ...
                              for (int p = 0; p < psize; ++p)
                                kernel_quarter.packet[p] =
                                  lhs.template loadPacket<QuarterPacket>(i + p +
                                                                           m,
                                                                         k);
          ...
          if (pack <= 0)
            {
              ...
              // products like real * complex, where we have to go half the
              // progress on the lhs in order to duplicate those operands to
              // address both real & imaginary parts on the rhs. This portion
              // will pack those half ones until they match the number expected
              // on the last peeling loop at this point (for the rhs).
