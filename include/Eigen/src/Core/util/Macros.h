...
#  if defined(__cpp_variable_templates) && \
    __cpp_variable_templates >= 201304 && EIGEN_MAX_CPP_VER >= 14
...
#  if EIGEN_MAX_CPP_VER >= 11 &&                                \
    (EIGEN_HAS_CXX11 || EIGEN_COMP_MSVC >= 1700) &&             \
...
namespace Eigen
{
  namespace internal
  {
    ...
    inline void
    assert_fail(const char *condition,
                const char *function,
                const char *file,
                int         line)
    {
      ...
      do                                                      \
        {                                                     \
          if (!Eigen::internal::copy_bool(x))                 \
            Eigen::internal::assert_fail(EIGEN_MAKESTRING(x), \
                                         __PRETTY_FUNCTION__, \
                                         __FILE__,            \
                                         __LINE__);           \
        }                                                     \
...
#define EIGEN_GENERIC_PUBLIC_INTERFACE(Derived)                                  \
  ...
  typedef typename Base::CoeffReturnType                                         \
    CoeffReturnType; /*!< \brief The return type for coefficient access.         \
                        \details Depending on whether the object allows direct   \
                        coefficient access (e.g. for a MatrixXd), this type is   \
                        either 'const Scalar&' or simply 'Scalar' for objects    \
