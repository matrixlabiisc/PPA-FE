...
namespace Eigen
{
   ...
   * EIGEN_DEFAULT_DENSE_INDEX_TYPE. \sa \blank \ref
...
#  endif
    ...
    // template<typename Scalar> struct random_default_impl<Scalar, false,
    // true>.
    template <typename>
...
#  endif
    ...
    /** \internal
     * Provides access to the number of elements in the object of as a
     * compile-time constant expression. It "returns" Eigen::Dynamic if the size
     * cannot be resolved at compile-time (default).
     *
 ...
 */
    ...
    {
      typedef
        typename Func::template result<Func(ArgType0, ArgType1)>::type type;
    };
    ...
    struct result_of<Func(ArgType0, ArgType1)>
    {
      ...
      typedef typename binary_result_of_select<Func,
                                               ArgType0,
                                               ArgType1,
    ...
    struct has_ReturnType
    {
      template <typename C>
      static meta_yes
      testFunctor(C const *, typename C::ReturnType const * = 0);
      template <typename C>
      static meta_no
      testFunctor(...);
    ...
    struct has_nullary_operator
    {
      ...
      static meta_yes
      testFunctor(C const *,
                  typename enable_if<(sizeof(return_ptr<C>()->
                                             operator()()) > 0)>::type * = 0);
      static meta_no
      testFunctor(...);
    ...
    /** \internal Computes the least common multiple of two positive integer A
     * and B at compile-time.
     */
    template <int  A,
              int  B,
              int  K    = 1,
    ...
    /** \internal Obtains a POD type suitable to use as storage for an object of
     * a size of at most Len bytes, aligned as specified by \c Align.
     */
