...
namespace Eigen
{
  namespace internal
  {
    ...
    // PacketBlockManagement specialization to take care of RowMajor order
    // without ifs.
    ...
    public:
      ...
      EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE void
                        storePacket(Index i, const PacketType &p) const
      {
        pscatter<Scalar, PacketType>(m_data + i * m_incr.value(),
                                     p,
                                     m_incr.value());
      }
    ...
    public:
      ...
      struct storePacketBlock_helper
      {
        ...
        EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE void
                          store(const blas_data_mapper<Scalar,
                                     Index,
                                     StorageOrder,
                                     AlignmentType,
                                     Incr> *   sup,
    ...
    /**
     * \c combine_scalar_factors extracts and multiplies factors from GEMM and
     * GEMV products. There is a specialization for booleans
     */
    ...
    EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE ResScalar
                                          combine_scalar_factors(const ResScalar &alpha,
                                                                 const Lhs &      lhs,
                                                                 const Rhs &      rhs)
    {
      return combine_scalar_factors_impl<ResScalar, Lhs, Rhs>::run(alpha,
                                                                   lhs,
                                                                   rhs);
    }
    template <typename ResScalar, typename Lhs, typename Rhs>
