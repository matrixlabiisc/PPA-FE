...
namespace Eigen
{
   ...
   * symbol \c EIGEN_DENSEBASE_PLUGIN.
   *
  ...
  public:
    ...
    enum
    {
       ...
       * expression might have. If this expression might have an arbitrarily
       * high number of rows, this value is set to \a Dynamic.
       *
       ...
       * case, we are dealing with a column-vector (if there is only one column)
       * or with a row-vector (if there is only one row). */
    ...
    {
      return Replicate<Derived, Dynamic, Dynamic>(derived(),
                                                  rowFactor,
                                                  colFactor);
    }
