...
// might exist for more recent versions too.
  ...
  namespace internal
  {
     ...
     * - otherwise, global_math_functions_filtering_base<T>::type is a typedef
     * for T.
     *
     * How it's used:
     ...
     * object array1+array2 has a complicated expression type, all what you want
     * to know is that it inherits ArrayBase. So we implement a partial
     * specialization of sin_impl for ArrayBase<Derived>. So we must make sure
     * to use sin_impl<ArrayBase<Derived> > and not sin_impl<Derived>, otherwise
     * our partial specialization won't be used. How does sin know that? That's
     ...
     * compilers. With GCC, it sure does work, but if you replace the typename
     * dummy by an integer template parameter, it doesn't work anymore!
     */
...
#define EIGEN_MATHFUNC_RETVAL(func, scalar)                         \
  typename Eigen::internal::func##_retval<                          \
    ...
    EIGEN_DEVICE_FUNC std::complex<T>
                      complex_sqrt(const std::complex<T> &a_x);
    ...
    template <typename T>
    EIGEN_DEVICE_FUNC std::complex<T>
                      complex_rsqrt(const std::complex<T> &a_x);
    ...
    // generating warnings on clang.  Here we explicitly cast the real
    // component.
...
// This seems to be fixed in VS 2019.
    ...
    template <typename Scalar,
              bool HasStdImpl = NumTraits<Scalar>::IsComplex ||
                                is_integral<Scalar>::value ||
...
#  endif
    ...
    {
      // fallback expm1 implementation in case there is no expm1(Scalar)
      // function in namespace of Scalar, or that there is no suitable
      // std::expm1 function available. Implementation attributed to Kahan. See:
      ...
      expm1(const Scalar &x)
      {
        ...
        EIGEN_USING_STD(log);
        Scalar logu = log(u);
        return numext::equal_strict(u, logu) ? u :
                                               (u - RealScalar(1)) * x / logu;
      }
    } // namespace std_fallback
    ...
    struct random_default_impl<Scalar, false, true>
    {
      ...
      run()
      {
        ...
        enum {rand_bits = meta_floor_log2<(unsigned int)(RAND_MAX) + 1>::value,
              ...
              shift =
                EIGEN_PLAIN_ENUM_MAX(0, int(rand_bits) - int(scalar_bits)),
              offset =
...
#  define SYCL_SPECIALIZE_FLOATING_TYPES_UNARY_FUNC_RET_TYPE(NAME,           \
                                                             FUNC,           \
                                                             RET_TYPE)       \
...
#  define SYCL_SPECIALIZE_BINARY_FUNC(NAME, FUNC, TYPE) \
     ...
     * but slightly faster for float/double and some compilers (e.g., gcc),
     * thanks to specializations when SSE is enabled.
     *
