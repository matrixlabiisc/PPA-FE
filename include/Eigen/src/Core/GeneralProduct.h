...
#  define EIGEN_GEMM_TO_COEFFBASED_THRESHOLD 20
  ...
  namespace internal
  {
    ...
    struct product_type
    {
      ...
      // _select enums and the typedef below, is to work around an internal
      // compiler error with gcc 4.1 and 4.2.
    private:
      enum
      {
   ...
   * large => call fast BLAS-like colmajor routine 2 - the matrix is row-major,
   * BLAS compatible and N is large => call fast BLAS-like rowmajor routine 3 -
   * all other cases are handled using a simple loop along the outer-storage
  ...
  namespace internal
  {
    ...
    struct gemv_dense_selector<OnTheRight, ColMajor, true>
    {
          ...
          const typename Dest::Scalar &alpha)
      {
        ...
        typedef typename conditional<Dest::IsVectorAtCompileTime,
                                     Dest,
        ...
        if (!MightCannotUseDest)
        ...
        else
          {
            ...
            ei_declare_aligned_stack_constructed_variable(ResScalar,
                                                          actualDestPtr,
                                                          dest.size(),
                                                          evalToDest ?
                                                            dest.data() :
