...
#  define EIGEN_DEBUG_UNALIGNED_STORE
    ...
    /** \internal Wrapper to ensure that multiple packet types can map to the
       same same underlying vector type. */
    ...
    // pselect uses a comparison to zero, so this should still work in most
    // cases. We don't have another option, since the scalar type requires
    // initialization.
    template <typename T>
    struct ptrue_impl<
      T,
      typename internal::enable_if<is_scalar<T>::value &&
...
#  define EIGEN_BINARY_OP_NAN_PROPAGATION(Type, Func) \
    ...
    /** \internal \returns the significant and exponent of the underlying
    ...
    /** \internal \returns a packet version of \a *from, (un-aligned masked
     * load) There is no generic implementation. We only have implementations
     * for specialized cases. Generic case should not be called.
     */
    template <typename Packet>
    ...
    pbroadcast4(const typename unpacket_traits<Packet>::type *a,
                Packet &                                      a0,
                Packet &                                      a1,
                Packet &                                      a2,
                Packet &                                      a3)
    ...
    template <typename Packet>
    EIGEN_DEVICE_FUNC inline void
    pbroadcast2(const typename unpacket_traits<Packet>::type *a,
                Packet &                                      a0,
                Packet &                                      a1)
    {
      a0 = pload1<Packet>(a + 0);
    ...
    /** \internal \returns a packet with constant coefficients \a a, e.g.: (x,
       0, x, 0), where x is the value of all 1-bits. */
    template <typename Packet>
    EIGEN_DEVICE_FUNC inline Packet
    ...
    /** \internal copy the packet \a from to \a *to, (un-aligned store with a
     * mask) There is no generic implementation. We only have implementations
     * for specialized cases. Generic case should not be called.
     */
    template <typename Scalar, typename Packet>
    EIGEN_DEVICE_FUNC inline
...
#  endif
  ...
  (EIGEN_COMP_GNUC || EIGEN_COMP_CLANG || EIGEN_COMP_ICC)
    ...
    /** \internal \returns the sum of the elements of upper and lower half of \a
     * a if \a a is larger than 4. For a packet {a0, a1, a2, a3, a4, a5, a6,
     * a7}, it returns a half packet {a0+a4, a1+a5, a2+a6, a3+a7} For
     * packet-size smaller or equal to 4, this boils down to a noop.
     */
    template <typename Packet>
    EIGEN_DEVICE_FUNC inline
    ...
    predux_mul(const Packet &a)
    {
      ...
      return predux_helper(a,
                           EIGEN_BINARY_OP_NAN_PROPAGATION(Scalar,
                                                           (pmul<Scalar>)));
    }
    ...
    predux_min(const Packet &a)
    {
      ...
      return predux_helper(
        a,
        EIGEN_BINARY_OP_NAN_PROPAGATION(Scalar, (pmin<PropagateFast, Scalar>)));
    }
    ...
    predux_max(const Packet &a)
    {
      ...
      return predux_helper(
        a,
        EIGEN_BINARY_OP_NAN_PROPAGATION(Scalar, (pmax<PropagateFast, Scalar>)));
    }
...
#undef EIGEN_BINARY_OP_NAN_PROPAGATION
    ...
    {
      // Dirty but generic implementation where "true" is assumed to be non 0
      // and all the sames. It is expected that "true" is either:
      //  - Scalar(1)
      //  - bits full of ones (NaN for floats),
      //  - or first bit equals to 1 (1 for ints, smallest denormal for floats).
      // For all these cases, taking the sum is just fine, and this boils down
      // to a no-op for scalars.
      typedef typename unpacket_traits<Packet>::type Scalar;
    ...
    /** \internal copy a packet with constant coefficient \a a (e.g., [a,a,a,a])
     * to \a *to. \a to must be 16 bytes aligned */
    // NOTE: this function must really be templated on the packet type (think
    // about different packet types for the same scalar type)
    template <typename Packet>
    inline void
    ...
    /** \internal \returns a packet version of \a *from.
     * Unlike ploadt, ploadt_ro takes advantage of the read-only memory path on
     * the hardware if available to speedup the loading of data that won't be
     * modified by the current computation.
     */
