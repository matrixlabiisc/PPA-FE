    ...
    namespace internal
    {
      ...
      public:
        ...
        struct virtual_pointer_t
        {
           ...
           * number
           */
          virtual_pointer_t(base_ptr_t u)
            : m_contents(u){};
        };
         ...
         * \throws std::out:of_range if the pointer is not found or pMap is
         * empty
         */
        typename pointerMap_t::iterator
        ...
        get_buffer(const virtual_pointer_t ptr)
        {
          ...
          // `buffer<>`. We can do this without the `buffer_mem` being a
          // pointer, as we only declare member variables in the base class
          // (`buffer_mem`) and not in the child class (`buffer<>).
          auto node = get_node(ptr);
          eigen_assert(node->first == ptr || node->first < ptr);
          eigen_assert(ptr < static_cast<virtual_pointer_t>(
                               node->second.m_size + node->first));
        ...
        /*
         * Returns the number of elements by which the given pointer is offset
        ...
        /* add_pointer.
         * Adds an existing pointer to the map and returns the virtual pointer
         * id.
         */
      ...
      private:
        ...
        /* Base address used when issuing the first virtual pointer, allows
         * users to specify alignment. Cannot be zero. */
      ...
      struct RangeAccess
      {
        static const auto global_access =
          cl::sycl::access::target::global_buffer;
        static const auto is_place_holder =
        ...
        RangeAccess(std::nullptr_t)
          : RangeAccess()
        {}
        // This template parameter must be removed and scalar_t should be
        // replaced
