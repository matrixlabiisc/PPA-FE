...
namespace Eigen
{
  namespace internal
  {
    ...
    // Functions for sqrt.
    // The EIGEN_FAST_MATH version uses the _mm_rsqrt_ps approximation and one
    // step of Newton's method, at a cost of 1-2 bits of precision as opposed to
    // the exact solution. It does not handle +inf, or denormalized numbers
    // correctly. The main advantage of this approach is not just speed, but
    // also the fact that it can be inlined and pipelined with other
    // computations, further reducing its effective latency. This is similar to
    // Quake3's fast inverse square root. For detail see here:
                                                                     ...
                                                                     psqrt<Packet4f>(const Packet4f &_x)
    {
      ...
      Packet4f denormal_mask =
        pandnot(pcmp_lt(_x,
                        pset1<Packet4f>((std::numeric_limits<float>::min)())),
                pcmp_lt(_x, pzero(_x)));
                                                                     ...
                                                                     prsqrt<Packet4f>(const Packet4f &_x)
    {
      ...
      // * x) * y_n). It is essential to evaluate the inner term like this
      // because forming y_n^2 may over- or underflow.
      Packet4f y_newton =
        pmul(y_approx,
             pmadd(y_approx, pmul(neg_half, y_approx), p4f_one_point_five));

      // Select the result of the Newton-Raphson step for positive normal
      // arguments. For other arguments, choose the output of the intrinsic.
      // This will return rsqrt(+inf) = 0, rsqrt(x) = NaN if x < 0, and rsqrt(x)
      // = +inf if x is zero or a positive denormalized float (equivalent to
      // flushing positive denormalized inputs to zero).
