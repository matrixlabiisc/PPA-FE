...
#define EIGEN_ARCH_DEFAULT_NUMBER_OF_REGISTERS 32
                        ...
                        plset<PacketXi>(const numext::int32_t &a)
    {
      ...
      for (int i = 0; i < packet_traits<numext::int32_t>::size; i++)
        c[i] = i;
      return svadd_s32_z(svptrue_b32(),
                         pset1<PacketXi>(a),
    ...
    pgather<numext::int32_t, PacketXi>(const numext::int32_t *from,
                                       Index                  stride)
    {
    ...
    pscatter<numext::int32_t, PacketXi>(numext::int32_t *to,
                                        const PacketXi & from,
                                        Index            stride)
    {
      // Indice format: {base=0, base+stride, base+stride*2, base+stride*3, ...}
                        ...
                        predux_mul<PacketXi>(const PacketXi &a)
    {
      ...
      // Extract the high half of the vector. Depending on the VL more
      // reductions need to be done
    ...
    ptranspose(PacketBlock<PacketXi, N> &kernel)
    {
      ...
      for (i = 0; i < N; i++)
        {
          svst1_scatter_s32index_s32(svptrue_b32(),
                                     buffer + i,
                                     stride_index,
                                     kernel.packet[i]);
        }
      for (i = 0; i < N; i++)
        {
          kernel.packet[i] =
            svld1_s32(svptrue_b32(),
                      buffer + i * packet_traits<numext::int32_t>::size);
        }
    }
                        ...
                        plset<PacketXf>(const float &a)
    {
      ...
      for (int i = 0; i < packet_traits<float>::size; i++)
        c[i] = i;
      return svadd_f32_z(svptrue_b32(),
                         pset1<PacketXf>(a),
    ...
    // Float comparisons in SVE return svbool (predicate). Use svdup to set
    // active lanes to 1 (0xffffffffu) and inactive lanes to 0.
    template <>
    EIGEN_STRONG_INLINE PacketXf
    ...
    {
      return svreinterpret_f32_u32(svand_u32_z(svptrue_b32(),
    ...
    {
      return svreinterpret_f32_u32(svorr_u32_z(svptrue_b32(),
                                               svreinterpret_u32_f32(a),
                                               svreinterpret_u32_f32(b)));
    }
    ...
    {
      return svreinterpret_f32_u32(sveor_u32_z(svptrue_b32(),
                                               svreinterpret_u32_f32(a),
                                               svreinterpret_u32_f32(b)));
    }
    ...
    {
      return svreinterpret_f32_u32(svbic_u32_z(svptrue_b32(),
                                               svreinterpret_u32_f32(a),
                                               svreinterpret_u32_f32(b)));
    }
    ...
    ptranspose(PacketBlock<PacketXf, N> &kernel)
    {
      ...
      for (i = 0; i < N; i++)
        {
          svst1_scatter_s32index_f32(svptrue_b32(),
                                     buffer + i,
                                     stride_index,
                                     kernel.packet[i]);
        }
