...
namespace Eigen
{
  namespace internal
  {
                                          ...
                                          pfrexp_generic(const Packet &a, Packet &exponent)
    {
      ...
      // To handle denormals, normalize by multiplying by
      // 2^(int(MantissaBits)+1).
    ...
    // Safely applies ldexp, correctly handles overflows, underflows and
    // denormals. Assumes IEEE floating point format.
    template <typename Packet>
    EIGEN_STRONG_INLINE EIGEN_DEVICE_FUNC Packet
                                          pldexp_generic(const Packet &a, const Packet &exponent)
    {
      ...
      // computations.
      //
      // Since 'a' and the output can be denormal, the maximum range of
      // 'exponent' to consider for a float is:
      //   -255-23 -> 255+23
      // Below -278 any finite float 'a' will become zero, and above +278 any
      // finite float will become inf, including when 'a' is the smallest
      // possible denormal.
      //
      ...
      // factors, since [exponent>>2] is much faster to compute that
      // [exponent/3].
      //
    ...
    struct pldexp_fast_impl
    {
                                                   ...
                                                   run(const Packet &a, const Packet &exponent)
      {
        ...
        const PacketI e =
          pcast<Packet, PacketI>(pmin(pmax(padd(exponent, bias), pzero(limit)),
                                      limit)); // exponent + 127
        // return a * (2^e)
        return pmul(
    ...
    // logarithm can be easily approximated by a polynomial centered on m=1 for
    // stability.
    ...
    //    (1) balancing the mantissa and exponent to the required bits of 2/pi
    //    are aligned on 8-bits, and (2) replicating the storage of the bits of
    //    2/pi.
    ...
    trig_reduce_huge(float xf, int *quadrant)
    {
      ...
      const uint64_t zero_dot_five =
        uint64_t(1) << 61; // 0.5 in 2.62-bit fixed-point foramt
      ...
      psincos_float(const Packet &_x)
    {
      ...
      const Packet cst_2oPI =
        pset1<Packet>(0.636619746685028076171875f); // 2/PI
...
// using "Extended precision modular arithmetic"
      ...
      // It provides a max error of 1ULP up to (with absolute_error
      // < 5.9605e-08):
      const float huge_th = ComputeSine ? 117435.992f : 71476.0625f;
      x = pmadd(y, pset1<Packet>(-1.57079601287841796875f), x);
      x = pmadd(y,
                pset1<Packet>(-3.1391647326017846353352069854736328125e-07f),
                x);
      x =
        pmadd(y,
              pset1<Packet>(
              ...
              x);
#else
      // Without true FMA, the previous set of coefficients maintain 1ULP
      // accuracy up to x<15.7 (for sin), but accuracy is immediately lost for
      // x>15.7. We thus use one more iteration to maintain 2ULPs up to
      // reasonably large inputs.

      // The following set of coefficients maintain 1ULP up to 9.43 and 14.16
      // for sin and cos respectively. and 2 ULP up to:
      ...
      EIGEN_OPTIMIZATION_BARRIER(x)
      x = pmadd(y,
                pset1<Packet>(1.62865035235881805419921875e-07),
                x); // = 0x342ee000
      x = pmadd(y,
                pset1<Packet>(
...
// The coefficients are: 0xbfc90f80, 0xb7354480, 0x2e74b9ee
      ...
      //    x = (0:0.0001:pi/4)';
      //    A = [x.^3 x.^5 x.^7];
      //    w = ((1.-(x/(pi/4)).^2).^5)*2000+1;         # weights trading
      //    relative accuracy c = (A'*diag(w)*A)\(A'*diag(w)*(sin(x)-x)); #
      //    weighted LS, linear coeff forced to 1 printf('%.64f\n
      //    %.64f\n%.64f\n', c(3), c(2), c(1))
      //
                                                                     ...
                                                                     psqrt_complex(const Packet &a)
    {
      ...
      // equation for a single complex square root of the number x + i*y. We
      // want to find real numbers u and v such that
      //    (u + i*v)^2 = x + i*y  <=>
      ...
      //    u = 0.5 * (y / v)
      //
      //  To avoid unnecessary over- and underflow, we compute sqrt(x^2 + y^2)
      //  as
      //     l = max(|x|, |y|) * sqrt(1 + (min(|x|, |y|) / max(|x|, |y|))^2) ,

      // In the following, without lack of generality, we have annotated the
      // code, assuming that the input is a packet of 2 complex numbers.
      //
      ...
      // prepare packet of (+∞,0*|y|) or (0*|y|,+∞), depending on the sign of
      // the infinite real part.
      ...
      return pselect(is_imag_inf,
                     imag_inf_result,
                     pselect(is_real_inf, real_inf_result, result));
    }

    // TODO(rmlarsen): The following set of utilities for double word arithmetic
    // should perhaps be refactored as a separate file, since it would be
    // generally useful for special function implementation etc. Writing the
    // algorithms in terms if a double word type would also make the code more
    // readable.
    ...
    // Formally, pow(x,y) = exp2(y * log2(x)), where exp2(x) is shorthand for
    // 2^x.
    // TODO(rmlarsen): We should probably add this as a packet up 'ppow', to
    // make it easier to specialize or turn off for specific types and/or
    // backends.x
    template <typename Packet>
    EIGEN_STRONG_INLINE Packet
                        generic_pow_impl(const Packet &x, const Packet &y)
    {
      ...
      // Compute the two terms {y * e_x, y * r_x} in f = y * log2(x) with
      // doubled precision using double word arithmetic.
      ...
      // Since r_z is in [-0.5;0.5], we compute the first factor to high
      // accuracy using a specialized algorithm. Multiplication by the second
      // factor can be done exactly using pldexp(), since it is an integer power
      // of 2.
                                                                     ...
                                                                     generic_pow(const Packet &x, const Packet &y)
    {
      ...
      const Packet invalid_negative_x =
        pandnot(pandnot(pandnot(x_is_neg, abs_x_is_inf), y_is_int),
                abs_y_is_inf);
      const Packet pow_is_one =
        por(por(x_is_one, y_is_zero),
            pand(x_is_neg_one,
                 por(abs_y_is_inf, pandnot(y_is_even, invalid_negative_x))));
      const Packet pow_is_nan =
        por(invalid_negative_x, por(x_is_nan, y_is_nan));
    ...
    struct ppolevl
    {
                                                   ...
                                                   run(const Packet &x, const typename unpacket_traits<Packet>::type coeff[])
      {
        ...
        return pmadd(ppolevl<Packet, N - 1>::run(x, coeff),
                     x,
