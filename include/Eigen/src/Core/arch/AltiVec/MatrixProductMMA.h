...
namespace Eigen
{
  namespace internal
  {
    ...
    template <typename DataMapper,
              ...
              int         N>
    EIGEN_ALWAYS_INLINE void
    storeComplexAccumulator(Index             i,
                            Index             j,
                            const DataMapper &data,
                            ...
                            __vector_quad *   accImag)
    {
      ...
      bload<DataMapper, Packetc, Index, accColsC, N, ColMajor>(tRes,
                                                               data,
                                                               i,
                                                               j);
      ...
      data.template storePacketBlock<Packetc, 4>(i + (N + 1) * accColsC,
                                                 j,
                                                 acc2);
    }

    // Defaults to float32, since Eigen still supports C++03 we can't use
    ...
    pgerMMA(__vector_quad *acc, const RhsPacket &a, const LhsPacket &b)
    {
      if (NegativeAccumulate)
        {
          __builtin_mma_xvf32gernp(acc,
                                   (__vector unsigned char)a,
                                   (__vector unsigned char)b);
        }
      else
        {
          __builtin_mma_xvf32gerpp(acc,
                                   (__vector unsigned char)a,
                                   (__vector unsigned char)b);
        }
    }
    ...
    pgerMMA(__vector_quad *acc, const __vector_pair &a, const Packet2d &b)
    {
      if (NegativeAccumulate)
        {
          __builtin_mma_xvf64gernp(acc,
                                   (__vector_pair)a,
                                   (__vector unsigned char)b);
        }
      else
        {
          __builtin_mma_xvf64gerpp(acc,
                                   (__vector_pair)a,
                                   (__vector unsigned char)b);
        }
    }
    ...
    template <typename Scalar,
              ...
              bool RhsIsReal>
    EIGEN_ALWAYS_INLINE void
    pgercMMA(__vector_quad *  accReal,
             __vector_quad *  accImag,
             const Packet &   lhsV,
             const Packet &   lhsVi,
             ...
             const RhsPacket &rhsVi)
    {
      ...
      if (LhsIsReal)
      ...
      else
        {
          if (!RhsIsReal)
            {
              pgerMMA<Packet, RhsPacket, ConjugateLhs == ConjugateRhs>(accReal,
                                                                       rhsVi,
                                                                       lhsVi);
              pgerMMA<Packet, RhsPacket, ConjugateRhs>(accImag, rhsVi, lhsV);
            }
    ...
    // This is necessary because ploadRhs for double returns a pair of vectors
    // when MMA is enabled.
    template <typename Scalar, typename Packet>
    EIGEN_ALWAYS_INLINE void
...
#define MICRO_MMA_ONE_PEEL                           \
  if (sizeof(Scalar) == sizeof(float))               \
    {                                                \
      MICRO_MMA_UNROLL_TYPE_PEEL(MICRO_MMA_WORK_ONE, \
                                 MICRO_MMA_LOAD_ONE, \
                                 RhsPacket);         \
    }                                                \
  else                                               \
...
#define MICRO_MMA_ONE                               \
  if (sizeof(Scalar) == sizeof(float))              \
    {                                               \
      MICRO_MMA_UNROLL_TYPE_ONE(MICRO_MMA_WORK_ONE, \
                                MICRO_MMA_LOAD_ONE, \
                                RhsPacket);         \
    }                                               \
  else                                              \
...
#define MICRO_MMA_STORE MICRO_MMA_UNROLL(MICRO_MMA_STORE_ONE)
    ...
    gemm_unrolled_MMA_iteration(const DataMapper &res,
...
#define MAX_MMA_UNROLL 7
          while (row + MAX_MMA_UNROLL * accCols <= rows)
            {
              gemm_unrolled_MMA_iteration<MAX_MMA_UNROLL,
                                          Scalar,
                                          Packet,
                                          RhsPacket,
                                          DataMapper,
...
#define MICRO_COMPLEX_MMA_STORE_ONE(iter)              \
  if (unroll_factor > iter)                            \
    {                                                  \
      storeComplexAccumulator<DataMapper,              \
                              Index,                   \
                              Packet,                  \
                              Packetc,                 \
                              accColsC,                \
...
#define MICRO_COMPLEX_MMA_STORE \
  MICRO_COMPLEX_MMA_UNROLL(MICRO_COMPLEX_MMA_STORE_ONE)
    ...
    template <int unroll_factor,
              ...
              bool        RhsIsReal>
    EIGEN_STRONG_INLINE void
    gemm_complex_unrolled_MMA_iteration(const DataMapper &res,
                                        const Scalar *    lhs_base,
                                        const Scalar *    rhs_base,
                                        Index             depth,
                                        Index             strideA,
    ...
    template <typename LhsScalar,
              ...
              bool        RhsIsReal>
    void
    gemm_complexMMA(const DataMapper &res,
...
#define MAX_COMPLEX_MMA_UNROLL 4
          while (row + MAX_COMPLEX_MMA_UNROLL * accCols <= rows)
            {
              gemm_complex_unrolled_MMA_iteration<MAX_COMPLEX_MMA_UNROLL,
                                                  Scalar,
                                                  Packet,
                                                  Packetc,
                                                  RhsPacket,
          ...
          switch ((rows - row) / accCols)
            {
              ...
              case 4:
                gemm_complex_unrolled_MMA_iteration<4,
                                                    Scalar,
                                                    Packet,
                                                    Packetc,
                                                    RhsPacket,
              ...
              case 3:
                gemm_complex_unrolled_MMA_iteration<3,
                                                    Scalar,
                                                    Packet,
                                                    Packetc,
                                                    RhsPacket,
              ...
              case 2:
                gemm_complex_unrolled_MMA_iteration<2,
                                                    Scalar,
                                                    Packet,
                                                    Packetc,
                                                    RhsPacket,
              ...
              case 1:
                gemm_complex_unrolled_MMA_iteration<1,
                                                    Scalar,
                                                    Packet,
                                                    Packetc,
                                                    RhsPacket,
