...
namespace Eigen
{
  namespace internal
  {
    ...
    // imaginary vector, this turned out to be faster than Eigen's usual
    // approach of having real/imaginary pairs on a single vector. This
    // constants then are responsible to extract from convert between Eigen's
    // and MatrixProduct approach.
    ...
    /**
     * Symm packing is related to packing of symmetric adjoint blocks, as
     *expected the packing leaves the diagonal real, whatever is below it is
     *copied from the respective upper diagonal element and conjugated. There's
     *no PanelMode available for symm packing.
     *
     * Packing in general is supposed to leave the lhs block and the rhs block
     *easy to be read by gemm using its respective rank-update instructions. The
     ...
     *64-bit elements.
     *
     * As mentioned earlier MatrixProduct breaks complex numbers into a real
     *vector and a complex vector so packing has to take that into account, at
     *the moment, we run pack the real part and then the imaginary part, this is
     *the main reason why packing for complex is broken down into several
     *different parts, also the reason why we endup having a float32/64 and
     *complex float32/64 version.
     **/
    ...
    template <typename Scalar, typename Index, int StorageOrder, int N>
    EIGEN_STRONG_INLINE void
    symm_pack_rhs_helper(Scalar *      blockB,
                         const Scalar *_rhs,
                         Index         rhsStride,
                         Index         rows,
                         Index         cols,
    ...
    template <typename Scalar, typename Index, int StorageOrder>
    EIGEN_STRONG_INLINE void
    symm_pack_lhs_helper(Scalar *      blockA,
                         const Scalar *_lhs,
                         Index         lhsStride,
                         Index         cols,
                         Index         rows)
    ...
    {
      void
      operator()(std::complex<float> *      blockB,
    ...
    struct symm_pack_lhs<std::complex<float>,
                         Index,
                         Pack1,
                         Pack2_dummy,
                         StorageOrder>
    {
      void
      operator()(std::complex<float> *      blockA,
    ...
    {
      void
      operator()(std::complex<double> *      blockB,
    ...
    template <typename Index, int Pack1, int Pack2_dummy, int StorageOrder>
    struct symm_pack_lhs<std::complex<double>,
                         Index,
                         Pack1,
                         Pack2_dummy,
                         StorageOrder>
     ...
     *gebp_kernel, this happens because on special occasions it fills part of
     *block with other parts of the matrix. Two variables control how PanelMode
     *should behave: offset and stride. The idea is that those variables
     *represent whatever is going to be the real offset and stride in the future
     *and this is what you should obey. The process is to behave as you would
     *with normal packing but leave the start of each part with the correct
     *offset and the end as well respecting the real stride the block will have.
     *Gebp is aware of both blocks stride and offset and behaves accordingly.
     **/
    ...
    {
      EIGEN_STRONG_INLINE void
      operator()(std::complex<Scalar> *blockA,
        ...
        for (; j + vectorSize <= rows; j += vectorSize)
          {
            ...
            rir +=
              ((PanelMode) ? (vectorSize * (2 * stride - depth)) : vectorDelta);
          }
            ...
            for (; i + vectorSize <= depth; i += vectorSize)
              {
                ...
                if (UseLhs)
                  {
                    bload<DataMapper, Packet, Index, 4, 0, StorageOrder>(block,
                                                                         lhs,
                                                                         j,
                                                                         i);
                  }
            ...
            for (; i < depth; i++)
              {
                ...
                bload<DataMapper, PacketC, Index, 2, 0, ColMajor>(cblock,
                                                                  rhs,
                                                                  i,
                                                                  j);
    ...
    // 512-bits rank1-update of acc. It can either positive or negative
    // accumulate (useful for complex gemm).
    ...
    template <int N,
              ...
              bool RhsIsReal>
    EIGEN_ALWAYS_INLINE void
    pgerc_common(PacketBlock<Packet, N> *accReal,
                 ...
                 const Packet *          rhsVi)
    {
      ...
      if (LhsIsReal)
      ...
      else
        {
          if (!RhsIsReal)
            {
              pger_common<Packet, ConjugateLhs == ConjugateRhs>(accReal,
                                                                lhsVi,
                                                                rhsVi);
              pger_common<Packet, ConjugateRhs>(accImag, lhsV, rhsVi);
            }
    ...
    template <int N,
              ...
              bool RhsIsReal>
    EIGEN_ALWAYS_INLINE void
    pgerc(PacketBlock<Packet, N> *accReal,
          PacketBlock<Packet, N> *accImag,
    ...
    EIGEN_ALWAYS_INLINE void
    loadPacketRemaining(const Scalar *lhs_ptr,
                        const Scalar *lhs_ptr_imag,
                        Packet &      lhsV,
                        Packet &      lhsVi,
                        Index         remaining_rows)
    ...
    // Load a PacketBlock, the N parameters make tunning gemm easier so we can
    ...
    template <typename Packet>
    EIGEN_ALWAYS_INLINE void
    pbroadcast4_old(const __UNPACK_TYPE__(Packet) * a,
                    Packet &a0,
                    Packet &a1,
                    Packet &a2,
                    Packet &a3)
    ...
    template <>
    EIGEN_ALWAYS_INLINE void
    pbroadcast4_old<Packet2d>(const double *a,
                              Packet2d &    a0,
                              Packet2d &    a1,
                              Packet2d &    a2,
                              Packet2d &    a3)
...
#define PEEL 7
    ...
    template <typename Scalar, typename Packet, typename Index>
    EIGEN_ALWAYS_INLINE void
    MICRO_EXTRA_COL(const Scalar *&         lhs_ptr,
                   ...
                   const Packet &    pAlpha)
    {
      ...
      const Scalar *lhs_ptr =
        lhs_base + row * strideA + remaining_rows * offsetA;
      PacketBlock<Packet, 1> accZero;
      ...
      for (; k < depth; k++)
        {
          Packet rhsV[1];
          rhsV[0] = pset1<Packet>(rhs_ptr[0]);
          pger<1, Scalar, Packet, Index, false>(&accZero,
                                                lhs_ptr,
                                                rhsV,
                                                remaining_rows);
          lhs_ptr += remaining_rows;
    ...
    template <typename Scalar,
              ...
              const Index accRows>
    EIGEN_ALWAYS_INLINE void
    MICRO_EXTRA_ROW(const Scalar *&         lhs_ptr,
                    const Scalar *&         rhs_ptr,
                   ...
                   const Packet &    pMask)
    {
      ...
      Index remaining_depth =
        (col + accRows < cols) ? depth : (depth & -accRows);
      Index k = 0;
      for (; k + PEEL <= remaining_depth; k += PEEL)
        {
          EIGEN_POWER_PREFETCH(rhs_ptr);
          ...
          for (int l = 0; l < PEEL; l++)
            {
              MICRO_EXTRA_ROW<Scalar, Packet, Index, accRows>(lhs_ptr,
                                                              rhs_ptr,
                                                              accZero,
                                                              remaining_rows);
            }
          ...
          for (; k < depth; k++)
            {
              Packet rhsV[4];
              pbroadcast4<Packet>(rhs_ptr, rhsV[0], rhsV[1], rhsV[2], rhsV[3]);
              pger<4, Scalar, Packet, Index, false>(&accZero,
                                                    lhs_ptr,
                                                    rhsV,
                                                    remaining_rows);
              lhs_ptr += remaining_rows;
...
#define MICRO_TYPE_PEEL4(func, func2, peel)                          \
  if (PEEL > peel)                                                   \
    {                                                                \
      ...
      pbroadcast4<Packet>(rhs_ptr + (accRows * peel),                \
                          rhsV##peel[0],                             \
                          rhsV##peel[1],                             \
                          rhsV##peel[2],                             \
                          rhsV##peel[3]);                            \
...
#define MICRO_STORE_ONE(iter)                                           \
  if (unroll_factor > iter)                                             \
    {                                                                   \
      ...
      res.template storePacketBlock<Packet, 4>(row + iter * accCols,    \
                                               col,                     \
                                               acc);                    \
    }
  ...
  if (unroll_factor > iter)                                             \
    {                                                                   \
      acc.packet[0] =                                                   \
        res.template loadPacket<Packet>(row + iter * accCols, col + 0); \
      bscale<Packet>(acc, accZero##iter, pAlpha);                       \
      res.template storePacketBlock<Packet, 1>(row + iter * accCols,    \
                                               col,                     \
                                               acc);                    \
    }
...
#define MICRO_COL_STORE MICRO_UNROLL(MICRO_COL_STORE_ONE)
    ...
    template <int unroll_factor,
              ...
              const Index accCols>
    EIGEN_STRONG_INLINE void
    gemm_unrolled_iteration(const DataMapper &res,
                            const Scalar *    lhs_base,
                            const Scalar *    rhs_base,
                            Index             depth,
                            Index             strideA,
    ...
    template <int unroll_factor,
              ...
              const Index accCols>
    EIGEN_STRONG_INLINE void
    gemm_unrolled_col_iteration(const DataMapper &res,
                                const Scalar *    lhs_base,
                                const Scalar *    rhs_base,
                                Index             depth,
                                Index             strideA,
...
#define MAX_UNROLL 6
      while (row + MAX_UNROLL * accCols <= rows)
        {
          gemm_unrolled_col_iteration<MAX_UNROLL,
                                      Scalar,
                                      Packet,
                                      DataMapper,
                                      Index,
...
#define MAX_UNROLL 6
          while (row + MAX_UNROLL * accCols <= rows)
            {
              gemm_unrolled_iteration<MAX_UNROLL,
                                      Scalar,
                                      Packet,
                                      DataMapper,
                                      Index,
...
#define PEEL_COMPLEX 3
      ...
      for (; k + PEEL_COMPLEX <= remaining_depth; k += PEEL_COMPLEX)
        {
          ...
          for (int l = 0; l < PEEL_COMPLEX; l++)
            {
              MICRO_COMPLEX_EXTRA_COL<Scalar,
                                      Packet,
                                      Index,
                                      accRows,
                                      ConjugateLhs,
                           ...
                           const Packet &    pMask)
    {
      ...
      for (; k + PEEL_COMPLEX <= remaining_depth; k += PEEL_COMPLEX)
        {
          ...
          for (int l = 0; l < PEEL_COMPLEX; l++)
            {
              MICRO_COMPLEX_EXTRA_ROW<Scalar,
                                      Packet,
                                      Index,
                                      accRows,
                                      ConjugateLhs,
      ...
      if ((remaining_depth == depth) && (rows >= accCols))
        {
          bload<DataMapper, Packetc, Index, accColsC, 0, ColMajor>(tRes,
                                                                   res,
                                                                   row,
                                                                   col);
          bscalec<Packet>(accReal,
                          accImag,
                          pAlphaReal,
                          pAlphaImag,
                          taccReal,
              ...
              for (Index j = 0; j < 4; j++)
                {
                  ...
                  acc2.packet[0] =
                    ...
                    acc0.packet[j];
                  res.template storePacketBlock<Packetc, 1>(row + 0,
                                                            col + j,
                                                            acc2);
                  if (remaining_rows > accColsC)
                    {
...
#define MICRO_COMPLEX_ONE_PEEL4                            \
  MICRO_COMPLEX_UNROLL_TYPE_PEEL(4,                        \
...
#define MICRO_COMPLEX_ONE4                                \
  MICRO_COMPLEX_UNROLL_TYPE_ONE(4,                        \
                                MICRO_COMPLEX_TYPE_PEEL4, \
                                MICRO_COMPLEX_WORK_ONE4,  \
                                MICRO_COMPLEX_LOAD_ONE);  \
  rhs_ptr_real += accRows;                                \
...
#define MICRO_COMPLEX_ONE_PEEL1                            \
  MICRO_COMPLEX_UNROLL_TYPE_PEEL(1,                        \
...
#define MICRO_COMPLEX_ONE1                                \
  MICRO_COMPLEX_UNROLL_TYPE_ONE(1,                        \
                                MICRO_COMPLEX_TYPE_PEEL1, \
                                MICRO_COMPLEX_WORK_ONE1,  \
                                MICRO_COMPLEX_LOAD_ONE);  \
  rhs_ptr_real += remaining_cols;                         \
...
#define MICRO_COMPLEX_COL_STORE \
  MICRO_COMPLEX_UNROLL(MICRO_COMPLEX_COL_STORE_ONE)
    ...
    template <int unroll_factor,
              ...
              bool        RhsIsReal>
    EIGEN_STRONG_INLINE void
    gemm_complex_unrolled_iteration(const DataMapper &res,
                                    const Scalar *    lhs_base,
                                    const Scalar *    rhs_base,
                                    Index             depth,
                                    Index             strideA,
    ...
    template <int unroll_factor,
              ...
              bool        RhsIsReal>
    EIGEN_STRONG_INLINE void
    gemm_complex_unrolled_col_iteration(const DataMapper &res,
                                        const Scalar *    lhs_base,
                                        const Scalar *    rhs_base,
                                        Index             depth,
                                        Index             strideA,
...
#define MAX_COMPLEX_UNROLL 3
      while (row + MAX_COMPLEX_UNROLL * accCols <= rows)
        {
          gemm_complex_unrolled_col_iteration<MAX_COMPLEX_UNROLL,
                                              Scalar,
                                              Packet,
                                              Packetc,
                                              DataMapper,
      ...
      switch ((rows - row) / accCols)
        {
          ...
          case 4:
            gemm_complex_unrolled_col_iteration<4,
                                                Scalar,
                                                Packet,
                                                Packetc,
                                                DataMapper,
          ...
          case 3:
            gemm_complex_unrolled_col_iteration<3,
                                                Scalar,
                                                Packet,
                                                Packetc,
                                                DataMapper,
          ...
          case 2:
            gemm_complex_unrolled_col_iteration<2,
                                                Scalar,
                                                Packet,
                                                Packetc,
                                                DataMapper,
          ...
          case 1:
            gemm_complex_unrolled_col_iteration<1,
                                                Scalar,
                                                Packet,
                                                Packetc,
                                                DataMapper,
...
#undef MAX_COMPLEX_UNROLL
    ...
    template <typename LhsScalar,
              ...
              bool        RhsIsReal>
    EIGEN_STRONG_INLINE void
    gemm_complex(const DataMapper &res,
...
#define MAX_COMPLEX_UNROLL 3
          while (row + MAX_COMPLEX_UNROLL * accCols <= rows)
            {
              gemm_complex_unrolled_iteration<MAX_COMPLEX_UNROLL,
                                              Scalar,
                                              Packet,
                                              Packetc,
                                              DataMapper,
...
#if MAX_COMPLEX_UNROLL > 4
              case 4:
                gemm_complex_unrolled_iteration<4,
                                                Scalar,
                                                Packet,
                                                Packetc,
                                                DataMapper,
...
#if MAX_COMPLEX_UNROLL > 3
              case 3:
                gemm_complex_unrolled_iteration<3,
                                                Scalar,
                                                Packet,
                                                Packetc,
                                                DataMapper,
...
#if MAX_COMPLEX_UNROLL > 2
              case 2:
                gemm_complex_unrolled_iteration<2,
                                                Scalar,
                                                Packet,
                                                Packetc,
                                                DataMapper,
...
#if MAX_COMPLEX_UNROLL > 1
              case 1:
                gemm_complex_unrolled_iteration<1,
                                                Scalar,
                                                Packet,
                                                Packetc,
                                                DataMapper,
          ...
          for (; col < cols; col++)
            {
              ...
              gemm_complex_unrolled_col<Scalar,
                                        Packet,
                                        Packetc,
                                        DataMapper,
                                        Index,
...
#undef advanceRows
    ...
    gemm_pack_lhs<double,
                  ...
                  ColMajor,
                  Conjugate,
                  PanelMode>::operator()(double *          blockA,
                                         const DataMapper &lhs,
                                         Index             depth,
                                         Index             rows,
                                         Index             stride,
    ...
    template <typename Index,
              ...
              bool PanelMode>
    void
    gemm_pack_rhs<double,
                  Index,
                  DataMapper,
                  nr,
                  ColMajor,
                  Conjugate,
                  PanelMode>::operator()(double *          blockB,
                                         const DataMapper &rhs,
                                         Index             depth,
                                         Index             cols,
                                         Index             stride,
    ...
    gemm_pack_lhs<float,
                  ...
                  RowMajor,
                  Conjugate,
                  PanelMode>::operator()(float *           blockA,
                                         const DataMapper &lhs,
                                         Index             depth,
                                         Index             rows,
                                         Index             stride,
    ...
    template <typename Index,
              ...
              bool Conjugate,
              bool PanelMode>
    struct gemm_pack_lhs<std::complex<float>,
                         Index,
                         DataMapper,
                         Pack1,
                         Pack2,
    ...
    template <typename Index,
              ...
              bool PanelMode>
    void
    gemm_pack_lhs<std::complex<float>,
                  Index,
                  DataMapper,
                  Pack1,
                  Pack2,
    ...
    gemm_pack_rhs<float,
                  ...
                  ColMajor,
                  Conjugate,
                  PanelMode>::operator()(float *           blockB,
                                         const DataMapper &rhs,
                                         Index             depth,
                                         Index             cols,
                                         Index             stride,
    ...
    template <typename Index,
              ...
              bool Conjugate,
              bool PanelMode>
    struct gemm_pack_rhs<std::complex<float>,
                         Index,
                         DataMapper,
                         nr,
                         ColMajor,
    ...
    template <typename Index,
              ...
              bool PanelMode>
    void
    gemm_pack_rhs<std::complex<float>,
                  Index,
                  DataMapper,
                  nr,
                  ColMajor,
    ...
    template <typename Index,
              ...
              bool Conjugate,
              bool PanelMode>
    struct gemm_pack_lhs<std::complex<double>,
                         Index,
                         DataMapper,
                         Pack1,
                         Pack2,
    ...
    template <typename Index,
              ...
              bool PanelMode>
    void
    gemm_pack_lhs<std::complex<double>,
                  Index,
                  DataMapper,
                  Pack1,
                  Pack2,
    ...
    template <typename Index,
              ...
              bool Conjugate,
              bool PanelMode>
    struct gemm_pack_rhs<std::complex<double>,
                         Index,
                         DataMapper,
                         nr,
                         ColMajor,
    ...
    template <typename Index,
              ...
              bool PanelMode>
    void
    gemm_pack_rhs<std::complex<double>,
                  Index,
                  DataMapper,
                  nr,
                  ColMajor,
                                          ...
                                          Index             offsetB)
    {
      ...
      void (*gemm_function)(const DataMapper &,
                            const float *,
                            const float *,
                            Index,
                            Index,
      ...
      gemm_function = &Eigen::internal::gemmMMA<float,
                                                Index,
                                                Packet,
                                                RhsPacket,
                                                DataMapper,
      ...
      if (__builtin_cpu_supports("arch_3_1") && __builtin_cpu_supports("mma"))
        ...
        {
          gemm_function = &Eigen::internal::gemm<float,
                                                 Index,
                                                 Packet,
                                                 RhsPacket,
                                                 DataMapper,
    ...
    template <typename Index,
              ...
              bool ConjugateLhs,
              bool ConjugateRhs>
    struct gebp_kernel<std::complex<float>,
                       std::complex<float>,
                       Index,
                       DataMapper,
                       mr,
                                          ...
                                          Index                      offsetB)
    {
...
#ifdef EIGEN_ALTIVEC_MMA_ONLY
      // generate with MMA only
      gemm_function = &Eigen::internal::gemm_complexMMA<std::complex<float>,
                                                        std::complex<float>,
                                                        std::complex<float>,
                                                        float,
                                                        Index,
      ...
      if (__builtin_cpu_supports("arch_3_1") && __builtin_cpu_supports("mma"))
        ...
        {
          gemm_function = &Eigen::internal::gemm_complex<std::complex<float>,
                                                         std::complex<float>,
                                                         std::complex<float>,
                                                         float,
                                                         Index,
                                          ...
                                          Index                      offsetB)
    {
...
#ifdef EIGEN_ALTIVEC_MMA_ONLY
      // generate with MMA only
      gemm_function = &Eigen::internal::gemm_complexMMA<float,
                                                        std::complex<float>,
                                                        std::complex<float>,
                                                        float,
                                                        Index,
      ...
      if (__builtin_cpu_supports("arch_3_1") && __builtin_cpu_supports("mma"))
        ...
        {
          gemm_function = &Eigen::internal::gemm_complex<float,
                                                         std::complex<float>,
                                                         std::complex<float>,
                                                         float,
                                                         Index,
                                          ...
                                          Index             offsetB)
    {
...
#ifdef EIGEN_ALTIVEC_MMA_ONLY
      // generate with MMA only
      gemm_function = &Eigen::internal::gemmMMA<double,
                                                Index,
                                                Packet,
                                                RhsPacket,
                                                DataMapper,
      ...
      if (__builtin_cpu_supports("arch_3_1") && __builtin_cpu_supports("mma"))
        ...
        {
          gemm_function = &Eigen::internal::gemm<double,
                                                 Index,
                                                 Packet,
                                                 RhsPacket,
                                                 DataMapper,
    ...
    template <typename Index,
              ...
              bool ConjugateLhs,
              bool ConjugateRhs>
    struct gebp_kernel<std::complex<double>,
                       std::complex<double>,
                       Index,
                       DataMapper,
                       mr,
    ...
    template <typename Index,
              ...
              bool ConjugateRhs>
    void
    gebp_kernel<std::complex<double>,
                std::complex<double>,
                Index,
                DataMapper,
                mr,
                                          ...
                                          Index                       offsetB)
    {
...
#ifdef EIGEN_ALTIVEC_MMA_ONLY
      // generate with MMA only
      gemm_function = &Eigen::internal::gemm_complexMMA<std::complex<double>,
                                                        std::complex<double>,
                                                        std::complex<double>,
                                                        double,
                                                        Index,
      ...
      if (__builtin_cpu_supports("arch_3_1") && __builtin_cpu_supports("mma"))
        ...
        {
          gemm_function = &Eigen::internal::gemm_complex<std::complex<double>,
                                                         std::complex<double>,
                                                         std::complex<double>,
                                                         double,
                                                         Index,
                                          ...
                                          Index                       offsetB)
    {
...
#ifdef EIGEN_ALTIVEC_MMA_ONLY
      // generate with MMA only
      gemm_function = &Eigen::internal::gemm_complexMMA<double,
                                                        std::complex<double>,
                                                        std::complex<double>,
                                                        double,
                                                        Index,
      ...
      if (__builtin_cpu_supports("arch_3_1") && __builtin_cpu_supports("mma"))
        {
          gemm_function =
            &Eigen::internal::gemm_complexMMA<double,
                                              std::complex<double>,
                                              std::complex<double>,
                                              double,
                                              Index,
        ...
        {
          gemm_function = &Eigen::internal::gemm_complex<double,
                                                         std::complex<double>,
                                                         std::complex<double>,
                                                         double,
                                                         Index,
