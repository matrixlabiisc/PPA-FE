...
namespace Eigen
{
  ...
  inline void
  umfpack_defaults(double control[UMFPACK_CONTROL],
                   std::complex<double>,
                   SuiteSparse_long)
  {
  ...
  // Report status
  inline void
  umfpack_report_status(double control[UMFPACK_CONTROL],
                        int    status,
                        double,
                        int)
  {
  ...
  inline void
  umfpack_report_control(double control[UMFPACK_CONTROL],
                         std::complex<double>,
                         int)
  {
  ...
  umfpack_symbolic(SuiteSparse_long           n_row,
                   ...
                   double                     Info[UMFPACK_INFO])
  {
    return umfpack_zl_symbolic(n_row,
                               n_col,
                               Ap,
                               Ai,
                               &numext::real_ref(Ax[0]),
  ...
  // Get Determinant
  inline int
  umfpack_get_determinant(double *Mx,
                          double *Ex,
                          void *  NumericHandle,
   ...
   * squared and full rank. The vectors or matrices X and B can be either dense
   * or sparse.
   *
   ...
   * the inexpensive makeCompressed() to get a compressed matrix. \tparam
  ...
  public:
    ...
    printUmfpackInfo()
    {
      eigen_assert(m_analysisIsOk &&
                   "UmfPackLU: you must first call analyzePattern()");
      umfpack_report_info(m_control.data(),
                          m_umfpackInfo.data(),
                          Scalar(),
                          StorageIndex());
    }
    ...
    printUmfpackStatus()
    {
      eigen_assert(m_analysisIsOk &&
                   "UmfPackLU: you must first call analyzePattern()");
      umfpack_report_status(m_control.data(),
                            m_fact_errorCode,
                            Scalar(),
                            StorageIndex());
    }
