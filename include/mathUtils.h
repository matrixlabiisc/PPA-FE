...
inline double factorial(unsigned int N){  
    ...
    if( N > 20 ) 
        throw std::out_of_range ("This factorial function can handle 0 to 20 range only!!");
...
inline double distance3d(const dealii::Point<3>& evalPoint, 
...
inline double norm3d(const std::vector<double>& x){

    return std::sqrt(x[0]*x[0] + x[1]*x[1] + x[2]*x[2]);
}

inline double norm3d(const std::array<double,3>& x){
