  ...
  if (d_dftParamsPtr->spinPolarized == 0)
    {
      ...
      auto psiProjected2 = matrixmatrixTmul(Minv_scaledOrbitalValues_FEnodes,
                                            n_dofs,
                                            totalDimOfBasis,
                                            CoeffNew,
                                            numOfKSOrbitals,
        ...
        if (cell->is_locally_owned())
          {
            ...
            (rhoValues1)[cellId] = std::vector<double>(numQuadPoints, 0.0);
            (rhoValues2)[cellId] = std::vector<double>(numQuadPoints, 0.0);
          }
      ...
      pcout << "Total Charge old and new "
            << totalCharge(d_dofHandlerPRefined, rhoOutValues) << " "
            << totalCharge(d_dofHandlerPRefined, &rhoValues1) << " "
            << totalCharge(d_dofHandlerPRefined, &rhoValues2) << std::endl;
      pcout << "New error with old methods= "
            << newRhoSpillFactor(d_dofHandlerPRefined,
                                 rhoOutValues,
                                 &rhoValues1)
            << std::endl;
      pcout << "New error with new methods= "
            << newRhoSpillFactor(d_dofHandlerPRefined,
                                 rhoOutValues,
                                 &rhoValues2)
            << std::endl;
