...
// check https://stackoverflow.com/questions/2268749/defining-global-constant-in-c

// the types in above are usually inferred, can also give default type T = double
...
// derived (user-defined) data types and containers like std::vector and deallii::Point?
// std::vector might have efficient move semantics 
// check: https://stackoverflow.com/questions/35032340/passing-containers-by-value-or-by-reference

// zeta is from the slater rules here

double hydrogenic2sOrbital(double zeta, double r){

	return 0.25 * sqrt(pow3(zeta)/(2*M_PI)) * (2 - zeta*r) * exp(-zeta*r/2); 
	
}

double hydrogenic2pxOrbital(double zeta, double r, double xrel){

	return 0.25 * sqrt(pow5(zeta)/(2*M_PI)) * xrel * exp(-zeta*r/2); 
	
}

double hydrogenic2pyOrbital(double zeta, double r, double yrel){

	return 0.25 * sqrt(pow5(zeta)/(2*M_PI)) * yrel * exp(-zeta*r/2); 
}

double hydrogenic2pzOrbital(double zeta, double r, double zrel){

	return 0.25 * sqrt(pow5(zeta)/(2*M_PI)) * zrel * exp(-zeta*r/2); 
}

double MOCO1(const dealii::Point<3>& evalPoint){
	...
	const double zeta1 = 3.25; // Slater-rule zeta for 2s, 2px, 2py, 2pz for Carbon
	const double zeta2 = 4.55; // Slater-rule zeta for 2s, 2px, 2py, 2pz for Oxygen
...
double MOCO2(const dealii::Point<3>& evalPoint){
...
double MOCO3(const dealii::Point<3>& evalPoint){
...
double MOCO4(const dealii::Point<3>& evalPoint){
...
double MOCO5(const dealii::Point<3>& evalPoint){
...
double MOCO6(const dealii::Point<3>& evalPoint){
...
double MOCO7(const dealii::Point<3>& evalPoint){
...
double MOCO8(const dealii::Point<3>& evalPoint){
...
assembleHamiltonianMatrixOfCO(){
	...
	std::vector<double> UpperTriaHvec{
		...
		-90.63, 20.73, 0, 0, -52.54, -32.39, 0, 0,
				-102.59, 0, 0, 59.35, 43.97, 0, 0,
					   -87.01, 0, 0, 0, -31.20, 0,
						  -87.01, 0, 0, 0, -31.20,
		           			-111.57, -12.40, 0, 0,
									-116.66, 0, 0,
...
void assembleCO_LCAO_MOorbitals
