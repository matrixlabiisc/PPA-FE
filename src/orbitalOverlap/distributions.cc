...
// both are normalized and approach Dirac delta when sigma and epsilon go to zero resp. 
...
double gaussian(double x, double mu, double sigma = 0.1)
{
	return std::exp( -(x - mu)*(x - mu)/(2*sigma*sigma) )/( sigma * std::sqrt(2*M_PI) );
}

double lorentzian(double x, double mu, double epsilon = 0.1)
{
